...
La temática de mi trabajo es el cáncer. Estuve realizando búsqueda de datos sobre distintos tipos de cáncer, en concreto los de esta sección los obtuve de Eurostat.\
...
Observamos que los países vienen descritos en forma de código de 2 letras y no con su nombre completo. <br>
...
Por otro lado, al descargar los datos había la opción de género "Total", "Male", "Female" observé que en cualquier caso proporcionaba un conjunto de datos cuyas observaciones eran primero las de Female, Male y Total. Incluimos el género en una nueva columna :
...
Nos damos cuenta que para algunos países hay años en los que no hay registros ni para hombres ni para mujeres (con lo que para el total tampoco, pues hemos visto anteriormente que por defecto en el total si faltaba el dato de Mujeres le imputaban el dato de Hombres). Eliminamos estos registros.
...
Posteriormente veremos que será necesario importar un dataset con la población sobre los países europeos, obtuve uno a nivel mundial de la página https://datos.bancomundial.org/indicador/sp.pop.GROW?locations=EU. Éste se encuentra en formato Excel. 
...
### Gráfico
A partir de los gráficos siguientes se observan ciertas relaciones entre las variables perimeter_mean y area_mean(lo que guarda sentido). También entre compactness_mean y concavity_mean.
...
```
<br>
Procedemos a analizarlas detenidamente.
<br>
```{r echo=TRUE}
...
Parece que Alemania es la que tiene en media mayor número de pacientes con casos de cáncer de estómago, seguido de Italia y España. <br>
...
Observamos que los países con menos casos son Islandia, Luxemburgo y Malta. Es evidente que en países con menor número de habitantes éste número será menor, sería conveniente hacer un estudio del número de casos/número de habitantes por país para ver la ratio. Nos traemos el dataset población de la 3ª sección de Importación.
...
-   Países como Francia que antes aparecían a la cabeza, se encuentran en la cola si miramos la ratio. Por otro lado, se observa el caso contrario en Letonia.
...
Cuando calculamos la correlación observamos números demasiado altos ya que, por ejemplo, estamos trabajando con la variable radio_mean pero tambien radio_se, radio_worst.. que están altamente relacionadas. Decidimos por tanto trabajar con el conjunto de las 10 predictoras  *_mean.
...
Parece que regresión logística tiene mejor precisión, kappa y sensibilidad que el modelo de randomForest. Sin embargo, observamos una mayor especificidad de éste segundo modelo. <br>
